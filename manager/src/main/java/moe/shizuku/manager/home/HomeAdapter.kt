...
class HomeAdapter(private val homeModel: HomeViewModel, private val appsModel: AppsViewModel) :
    ...
    private var lastStatus: ServiceStatus? = null
    private var lastGrantedCount = -1
    ...
    fun updateData() {
        ...
        if (status == lastStatus && grantedCount == lastGrantedCount) return
        lastStatus = status
        ...
        if (isPrimaryUser) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R || EnvironmentUtils.getAdbTcpPort() > 0) {
                addItem(StartWirelessAdbViewHolder.CREATOR, running, ID_START_WADB)
            }
